...
namespace source2sdk::client
{
    ...
    public:
        ...
        entity2::GameTime_t m_flDashCastStartTime; // 0xc94        
        // metadata: MNetworkEnable
        // metadata: MNetworkUserGroup "LocalPlayerOwnerExclusive"
        Vector m_vDashCastDir; // 0xc98        
        [[maybe_unused]] std::uint8_t pad_0xca4[0x3c];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadelBaseDashCastAbility because it is not a standard-layout class
    static_assert(sizeof(CCitadelBaseDashCastAbility) == 0xce0);
};
