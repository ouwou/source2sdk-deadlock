...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t m_nBounces; // 0x8c8        
        entity2::GameTime_t m_tNextDetonateTime; // 0x8cc        
        ...
        char m_vecProjectileHitTargets[0x18]; // 0x8d0        
        [[maybe_unused]] std::uint8_t pad_0x8e8[0x28];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in C_Citadel_Projectile_Viscous_GooGrenade because it is not a standard-layout class
    static_assert(sizeof(C_Citadel_Projectile_Viscous_GooGrenade) == 0x910);
};
