...
namespace source2sdk::client
{
    ...
    public:
        ...
        char m_IcePathModifier[0x10]; // 0x1550        
        // metadata: MPropertyStartGroup "Gameplay"
        float m_flMomentumDecayRate; // 0x1560        
        float m_flMomentumWeight; // 0x1564        
        float m_flMaxPitchChange; // 0x1568        
        float m_flMaxPitchUp; // 0x156c        
        float m_flMaxPitchDown; // 0x1570        
        float m_flMaxHeight; // 0x1574        
        float m_flForwardAngleBias; // 0x1578        
        [[maybe_unused]] std::uint8_t pad_0x157c[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_IcePathVData because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_IcePathVData) == 0x1580);
};
