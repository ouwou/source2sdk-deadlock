...
namespace source2sdk::client
{
    ...
    {
    public:
        [[maybe_unused]] std::uint8_t pad_0xc80[0x150]; // 0xc80
        bool m_bZoomed; // 0xdd0        
        // metadata: MNetworkEnable
        bool m_bAirCast; // 0xdd1        
        [[maybe_unused]] std::uint8_t pad_0xdd2[0x2]; // 0xdd2
        // metadata: MNetworkEnable
        Vector m_vBeamAimPos; // 0xdd4        
        // metadata: MNetworkEnable
        // metadata: MNetworkEncoder "qangle"
        // metadata: MNetworkChangeCallback "OnBebopBeamAnglesChanged"
        QAngle m_angBeamAngles; // 0xde0        
        [[maybe_unused]] std::uint8_t pad_0xdec[0xc]; // 0xdec
        bool m_bNeedsBeamReset; // 0xdf8        
        [[maybe_unused]] std::uint8_t pad_0xdf9[0x7];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_Bebop_LaserBeam because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_Bebop_LaserBeam) == 0xe00);
};
