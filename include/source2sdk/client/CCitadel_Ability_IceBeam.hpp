...
namespace source2sdk::client
{
    ...
    {
    public:
        [[maybe_unused]] std::uint8_t pad_0xc98[0x618]; // 0xc98
        entity2::GameTime_t m_flNextDamageTick; // 0x12b0        
        Vector m_vStart; // 0x12b4        
        Vector m_vEnd; // 0x12c0        
        [[maybe_unused]] std::uint8_t pad_0x12cc[0x3c]; // 0x12cc
        ...
        char m_vecEntitiesHit[0x18]; // 0x1308        
        ...
        [[maybe_unused]] std::uint8_t pad_0x133c[0xc]; // 0x133c
        bool m_bNeedsBeamReset; // 0x1348        
        [[maybe_unused]] std::uint8_t pad_0x1349[0x7];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_IceBeam because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_IceBeam) == 0x1350);
};
