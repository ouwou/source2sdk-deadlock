...
namespace source2sdk::client
{
    ...
    {
    public:
        Vector m_vecAimPos; // 0xc80        
        Vector m_vecAimNormal; // 0xc8c        
        float m_flPushForce; // 0xc98        
        [[maybe_unused]] std::uint8_t pad_0xc9c[0x18c];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_Thumper_1 because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_Thumper_1) == 0xe28);
};
