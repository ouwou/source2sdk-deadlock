...
namespace source2sdk::client
{
    ...
    public:
        ...
        char m_hAbilityToTrigger[0x4]; // 0xc80        
        // metadata: MNetworkEnable
        // metadata: MNetworkUserGroup "LocalPlayerOwnerExclusive"
        entity2::GameTime_t m_SwappedToTime; // 0xc84        
        [[maybe_unused]] std::uint8_t pad_0xc88[0x8];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadelBaseTriggerAbility because it is not a standard-layout class
    static_assert(sizeof(CCitadelBaseTriggerAbility) == 0xc90);
};
