...
namespace source2sdk::client
{
    ...
    public:
        ...
        char m_vecLockonTargets[0x50]; // 0xd98        
        // metadata: MNetworkEnable
        entity2::GameTime_t m_LockOnStartTime; // 0xde8        
        [[maybe_unused]] std::uint8_t pad_0xdec[0x4]; // 0xdec
        client::ParticleIndex_t m_nTargetingLightEffect; // 0xdf0        
        [[maybe_unused]] std::uint8_t pad_0xdf4[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadelBaseLockonAbility because it is not a standard-layout class
    static_assert(sizeof(CCitadelBaseLockonAbility) == 0xdf8);
};
