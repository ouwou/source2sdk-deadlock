...
namespace source2sdk::server
{
    ...
    {
    public:
        [[maybe_unused]] std::uint8_t pad_0x9a8[0x14]; // 0x9a8
        ...
        server::CCitadelRegenComponent m_CCitadelRegenComponent; // 0x9c0        
        
        // Datamap fields:
        // CHandle< CBaseEntity > m_hAbilityOwner; // 0x9b0
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CNPC_SimpleAnimatingAI because it is not a standard-layout class
    static_assert(sizeof(CNPC_SimpleAnimatingAI) == 0xb40);
};
