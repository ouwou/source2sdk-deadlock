...
namespace source2sdk::server
{
    ...
    public:
        ...
        [[maybe_unused]] std::uint8_t pad_0x5e1[0x12f]; // 0x5e1
        client::ChatIgnoreType_t m_iIgnoreGlobalChat; // 0x710        
        float m_flLastPlayerTalkTime; // 0x714        
        float m_flLastEntitySteadyState; // 0x718        
        int32_t m_nAvailableEntitySteadyState; // 0x71c        
        bool m_bHasAnySteadyStateEnts; // 0x720        
        [[maybe_unused]] std::uint8_t pad_0x721[0xf]; // 0x721
        // metadata: MNetworkEnable
        // metadata: MNetworkEncoder "fixed64"
        // metadata: MNetworkChangeCallback "OnSteamIDChanged"
        uint64_t m_steamID; // 0x730        
        // metadata: MNetworkEnable
        uint32_t m_iDesiredFOV; // 0x738        
        [[maybe_unused]] std::uint8_t pad_0x73c[0x2c];
    ...
    static_assert(sizeof(CBasePlayerController) == 0x768);
};
