...
namespace source2sdk::server
{
    ...
    public:
        ...
        [[maybe_unused]] std::uint8_t pad_0x1848[0xc]; // 0x1848
        Vector m_vecHome; // 0x1854        
        [[maybe_unused]] std::uint8_t pad_0x1860[0x10]; // 0x1860
        int32_t m_nAbilityLevel; // 0x1870        
        [[maybe_unused]] std::uint8_t pad_0x1874[0x2c];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CNPC_SurveillanceDrone because it is not a standard-layout class
    static_assert(sizeof(CNPC_SurveillanceDrone) == 0x18a0);
};
