...
namespace source2sdk::server
{
    ...
    public:
        ...
        [[maybe_unused]] std::uint8_t pad_0x1630[0xc]; // 0x1630
        Vector m_vecHome; // 0x163c        
        [[maybe_unused]] std::uint8_t pad_0x1648[0x10]; // 0x1648
        int32_t m_nAbilityLevel; // 0x1658        
        [[maybe_unused]] std::uint8_t pad_0x165c[0x2c];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CNPC_SurveillanceDrone because it is not a standard-layout class
    
    static_assert(sizeof(CNPC_SurveillanceDrone) == 0x1688);
};
