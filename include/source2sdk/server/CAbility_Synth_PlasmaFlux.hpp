...
namespace source2sdk::server
{
    ...
    {
    public:
        [[maybe_unused]] std::uint8_t pad_0xaf0[0x10]; // 0xaf0
        bool m_bTeleported; // 0xb00        
        ...
        char m_vecUniqueHitList[0x18]; // 0xb08        
        Vector m_vLastValidTeleportPosition; // 0xb20        
        entity2::GameTime_t m_flProjectileLaunchTime; // 0xb2c        
        entity2::GameTime_t m_flProjectileExpireTime; // 0xb30        
        ...
        char m_hActiveProjectile[0x4]; // 0xb34        
        [[maybe_unused]] std::uint8_t pad_0xb38[0x158];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CAbility_Synth_PlasmaFlux because it is not a standard-layout class
    static_assert(sizeof(CAbility_Synth_PlasmaFlux) == 0xc90);
};
