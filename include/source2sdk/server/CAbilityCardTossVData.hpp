...
namespace source2sdk::server
{
    ...
    public:
        ...
        char m_SummonedCard[0xe0]; // 0x1630        
        // metadata: MPropertyStartGroup "Sounds"
        CSoundEventName m_strCardTossSound; // 0x1710        
        CSoundEventName m_strCardSummonSound; // 0x1720        
        // metadata: MPropertyStartGroup "+Card Toss Properties"
        float m_flSummonedCardStartSideOffset; // 0x1730        
        float m_flSummonedCardSideOffsetStep; // 0x1734        
        float m_flSummonedCardForwardOffset; // 0x1738        
        float m_flSummonedCardVerticalOffset; // 0x173c        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CAbilityCardTossVData because it is not a standard-layout class
    static_assert(sizeof(CAbilityCardTossVData) == 0x1740);
};
