...
namespace source2sdk::server
{
    ...
    public:
        ...
        server::CCitadelAbilityComponent m_CCitadelAbilityComponent; // 0x64c8        
        [[maybe_unused]] std::uint8_t pad_0x66d0[0x4]; // 0x66d0
        // metadata: MNetworkEnable
        bool m_bPickedUp; // 0x66d4        
        [[maybe_unused]] std::uint8_t pad_0x66d5[0x3];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadelItemPickupRejuv because it is not a standard-layout class
    static_assert(sizeof(CCitadelItemPickupRejuv) == 0x66d8);
};
