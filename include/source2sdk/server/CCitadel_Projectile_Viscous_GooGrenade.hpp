...
namespace source2sdk::server
{
    ...
    {
    public:
        int32_t m_nBounces; // 0x850        
        entity2::GameTime_t m_tNextDetonateTime; // 0x854        
        ...
        char m_vecProjectileHitTargets[0x18]; // 0x858        
        [[maybe_unused]] std::uint8_t pad_0x870[0x28];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Projectile_Viscous_GooGrenade because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Projectile_Viscous_GooGrenade) == 0x898);
};
