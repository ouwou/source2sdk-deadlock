...
namespace source2sdk::server
{
    ...
    public:
        ...
        char m_hWall[0x4]; // 0xaf0        
        Vector vecDir; // 0xaf4        
        client::ParticleIndex_t m_hChargingParticle; // 0xb00        
        Vector m_vSpawnPos; // 0xb04        
        QAngle m_qAngles; // 0xb10        
        // metadata: MNetworkEnable
        bool m_bAirCast; // 0xb1c        
        [[maybe_unused]] std::uint8_t pad_0xb1d[0x18b];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_Chrono_TimeWall because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_Chrono_TimeWall) == 0xca8);
};
