...
namespace source2sdk::server
{
    ...
    public:
        ...
        char m_ActiveCastParticle[0xe0]; // 0x1550        
        // metadata: MPropertyStartGroup "+Defer Properties"
        float m_flDeferredDamageApplicationInterval; // 0x1630        
        [[maybe_unused]] std::uint8_t pad_0x1634[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CAbilityShivDeferDamageVData because it is not a standard-layout class
    static_assert(sizeof(CAbilityShivDeferDamageVData) == 0x1638);
};
