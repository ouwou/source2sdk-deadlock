...
namespace source2sdk::server
{
    ...
    public:
        ...
        char m_DebuffModifier[0x10]; // 0x1598        
        ...
        char m_BuffParticle[0xe0]; // 0x1788        
        // metadata: MPropertyGroupName "Gameplay"
        float m_flTeleportDistance; // 0x1868        
        float m_flVelocityScale; // 0x186c        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CItemPhantomStrike_VData because it is not a standard-layout class
    static_assert(sizeof(CItemPhantomStrike_VData) == 0x1870);
};
