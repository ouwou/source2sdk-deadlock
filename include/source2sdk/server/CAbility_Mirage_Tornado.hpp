...
namespace source2sdk::server
{
    ...
    {
    public:
        Vector m_vLastValidMovementPosition; // 0xaf0        
        ...
        char m_hActiveProjectile[0x4]; // 0xafc        
        [[maybe_unused]] std::uint8_t pad_0xb00[0x188];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CAbility_Mirage_Tornado because it is not a standard-layout class
    static_assert(sizeof(CAbility_Mirage_Tornado) == 0xc88);
};
